...
pub const UniqueStorage = struct {
    alloc: Allocator,
    ...
    pub fn init(alloc: Allocator, child_allocator: Allocator) !Self {
        return UniqueStorage.initCapacity(alloc, child_allocator, 0);
    }
...
test "string UniqueStorage basic usage" {
    const alloc = std.testing.allocator;

    var ss = try UniqueStorage.init(alloc, std.heap.page_allocator);
    defer ss.deinit();
